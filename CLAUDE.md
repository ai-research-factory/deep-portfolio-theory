...
### Phase 4: Deep Portfolio戦略の実装 [Track ]

**Track**:  (A=論文再現 / B=近傍改善 / C=独自探索)
**ゴール**: AutoencoderをWalk-Forwardの枠組みに統合し、Deep Portfolio戦略のバックテストを実行する。

**具体的な作業指示**:
1. `src/strategies/deep_portfolio.py`に`DeepPortfolioStrategy`クラスを実装します。
2. このクラスは`generate_weights`メソッド内で、与えられた学習データを用いてAutoencoderモデル（Phase 1で実装）を学習させます。
3. 学習後、デコーダの最初の層の重み（`decoder[0].weight`）をファクターポートフォリオとして抽出します。
4. 論文の基本アプローチに従い、これらのファクターポートフォリオを均等加重（equal-weight）で合成し、最終的な資産ウェイトを計算します。ウェイトは合計が1になるように正規化してください。
5. `scripts/run_deep_portfolio.py`を作成し、`WalkForwardValidator`と`DeepPortfolioStrategy`を使ってバックテストを実行し、結果のポートフォリオ日次リターンを`reports/cycle_4/deep_portfolio_ae_returns.json`に保存します。

**期待される出力ファイル**:
- src/strategies/deep_portfolio.py
- scripts/run_deep_portfolio.py
- reports/cycle_4/deep_portfolio_ae_returns.json

**受入基準 (これを全て満たすまで完了としない)**:
- deep_portfolio_ae_returns.jsonが生成され、リターン時系列データが保存されている。
- Walk-Forwardの各ステップでAutoencoderの学習が実行されることをログで確認できる。
...
## スコア推移
Cycle 1: 45% → Cycle 2: 45% → Cycle 3: 45%
...
### マネージャー指示 (次のアクション)
1. 【REPLAN: 仕様整合修正】
primaryBlocker: 論文の実験設定からの大幅な逸脱（特にデータ頻度とユニバース）により、ベースラインである1/Nポートフォリオにすら劣後している問題
...
完了条件: Walk-Forwardバックテストにおいて、論文の実験設定（月次リターン、S&P500相当のユニバース）に準拠した実装で、Deep Portfolio戦略のシャープレシオが1/Nポートフォリオを上回る。`reports/backtest_summary.json` にて `deep_portfolio.sharpe > one_over_n.sharpe` が `true` となること。
2. 【最優先】src/data_loader.py内のデータ処理パイプラインを修正し、学習および評価に使用するリターンを日次から月次に変更する。具体的には、日次価格データから月の最終営業日の価格を抽出し、それを用いて月次リターンを計算するロジックを実装する。
3. 【重要】configs/universe.ymlで定義されている銘柄ユニバースを、現在の82銘柄からS&P500構成銘柄（またはそれに準ずる流動性の高い大型株 約500銘柄）に拡大する。データ取得スクリプト `scripts/get_data.py` を修正し、新しいユニバースに対応させる。
4. 【推奨】Audit不合格の原因となっている項目を修正し、テストが全てパスするようにする。特に、`src/portfolio.py`内の`MinimumVariancePortfolio`クラスにおける極端なウェイト集中の問題を緩和するため、各銘柄のウェイトに上限(例: 0.2)と下限(例: 0.0)の制約を追加する。
