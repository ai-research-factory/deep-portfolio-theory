...
### Phase 5: パフォーマンス評価指標とレポート作成 [Track ]

**Track**:  (A=論文再現 / B=近傍改善 / C=独自探索)
**ゴール**: 全戦略のパフォーマンスを計算・比較するレポートを生成する。

**具体的な作業指示**:
1. `src/evaluation/metrics.py`を作成します。年率リターン、年率ボラティリティ、シャープレシオ、ソルティノレシオ、最大ドローダウンを計算する関数を実装してください。
2. `scripts/generate_report.py`を作成します。
3. このスクリプトは`reports/cycle_3`と`reports/cycle_4`からベンチマークとDeep Portfolioのリターンデータを読み込みます。
4. `metrics.py`の関数を使い、各戦略のパフォーマンス指標を計算します。
5. 結果を整形し、`reports/cycle_5/performance_summary.md`にMarkdown形式のテーブルとして保存します。

**期待される出力ファイル**:
- src/evaluation/metrics.py
- scripts/generate_report.py
- reports/cycle_5/performance_summary.md

**受入基準 (これを全て満たすまで完了としない)**:
- performance_summary.mdが生成され、3つの戦略（1/N, MinVar, DeepPortfolio）の5つの評価指標が記載されたテーブルが含まれている。
- シャープレシオの値が妥当な範囲（-2から+3など）に収まっている。
...
## スコア推移
Cycle 1: 45% → Cycle 2: 45% → Cycle 3: 45% → Cycle 4: 45%
...
### マネージャー指示 (次のアクション)
1. 【REPLAN: ブロッカー修正】
primaryBlocker: 因子ポートフォリオ抽出ロジックの根本的な実装ミス

上記のブロッカーを最優先で解決してください。

完了条件: `tests/test_deep_portfolio.py` に、論文の式(4) `w_f = W2 @ W1` に基づく因子ポートフォリオの重み計算を検証するテストケースを追加し、パスすること。具体的には、`decoder[0].weight` (次元が不正) ではなく、エンコーダの重み `W1` とデコーダの重み `W2` の行列積が正しく計算され、ポートフォリオ重みとして適用されることを確認する。
2. 【最優先】`src/strategies/deep_portfolio.py`内の因子ポートフォリオ重み計算ロジックを全面的に再検証・修正する。現在の`decoder[0].weight`を使用している箇所を、論文の記述`W_f = W_2 @ W_1`（エンコーダの重みとデコーダの重みの行列積）に修正する。`technical_findings.md`の指摘に基づき、`W_1`と`W_2`の次元が正しく行列積を計算可能であることを確認するデバッグログを追加し、修正が正しいことを証明する単体テストを`tests/test_deep_portfolio.py`に実装する。
3. 【重要】`src/strategies/deep_portfolio.py`のAutoencoderモデルにL2正則化を導入する。`n < p` 問題による過学習を抑制するため、エンコーダとデコーダの両方の`Dense`層に`kernel_regularizer=l2(0.01)`を追加する。正則化パラメータは暫定値とし、今後のサイクルでチューニング対象とする。
4. 【推奨】データユニバースの小ささ（82銘柄）が論文（S&P 500）との大きな乖離点であるため、影響を調査する。`notebooks/`に新規ノートブック`05_universe_bias_analysis.ipynb`を作成し、現在の82銘柄がS&P 500のどのセクターに偏っているか、またボラティリティや時価総額の分布にどのようなバイアスがあるかを可視化・分析する。
